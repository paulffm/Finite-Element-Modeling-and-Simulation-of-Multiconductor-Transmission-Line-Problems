...
A current of :math:`I` = 16 A is homogeneously distributed along the cross-section of a cylindrical
copper wire with radius :math:`r_1` = 2 mm and length :math:`\ell_z` = 300 mm, aligned with the :math:`z`-direction. The
wire is enclosed within a cylindrical, non-conducting insulating shell with inner radius :math:`r_1`, outer
radius :math:`r_2` = 3.5 mm and with permeability :math:`\mu = 5\mu_0`, where :math:`\mu_0` is the permeability of
vacuum. The outer surface of the insulation shell is considered as a perfect electric conductor.

Basic structure:
1) Class WireProblem handles the modelling and problem formulation. Check the method create_problem
to see how to build the model using the Pyrit framework in Gmsh.
2) The static solver is initialized and solves the problem.
3) Post processing capabilities are shown, incl. comparison to analytic solution and convergence study.
...
convergence_oder = np.polyfit(np.log(element_sizes), np.log(rel_error), 1)[0]

_, ax = plt.subplots()
ax.loglog(element_sizes, rel_error, label="Error")
ax.legend()
ax.set_title(f"Relative error. Convergence of order {convergence_oder:.2f}")
plt.show()
